...
function handlePlayerOnClick(e, d) {
    ...
    if (element.classed('selected')) {
        ...
        let playerData = {}
        playerData.name = d
        playerData.country = playerCountryMap.get(d); // Pull the players name from the map and save the country (to later highlight nodes)
...
function updateCountry(currentPlayer, deselect) {
    ...
    selectedCountries.forEach(function (_, key) {
        div.append('img')
            ...
            .style('width', '125px')
            .style('height', '125px')
            .on('mouseover', mouseOverFlag)
            .on('mouseout', mouseOutFlag)
            .text(key); // Set the "text" of this node as the country
    });
}


function mouseOverFlag(e, d){
